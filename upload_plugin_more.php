 ...
 *
 */
class upload_plugin_more {
	/**
	 * 输出上传一个文件的前台代码
	 *
	 * @param String $accept_upload_ajax_url: 响应上传ajax的url地址
	 * @param Array $old_src: 已有的文件地址
	 * @return String $str
	 */
	public static function get_upload_more_file_html($accept_upload_ajax_url='',$old_src=array()) {	    
		//检查参数
		$accept_upload_ajax_url = $accept_upload_ajax_url=='' ? 'accept_all_type_more.php' : $accept_upload_ajax_url;
		//设置一个唯一标识
		...
		//输入框
		$str .= "<input id='upload_file_$sign' type='file' name='upload_file_$sign'.'[]' multiple/>";
		//上传进度显示
		...
		//若数据库里有文件地址，则直接显示
	if(!empty($old_src) && count($old_src)>0) {
